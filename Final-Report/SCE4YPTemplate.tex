...
%TODO: Write this into paragraph form
...
% TODO: Reference for layer: http://www.oreilly.com/programming/free/files/software-architecture-patterns.pdf
	% The reference is a book, need page 9 (page '1' of the book content)
...
{fig:hardware-block} is a block diagram of the initial design architecture of the lock software.
...
\begin{figure}
    ...
    \caption{Sequence diagram of the application requesting sample data after loggin in}
...
The authentication modules in this project were powered by Arduino IDE-compatible microcontrollers. The 
...
PN532 NFC shield to read NFC data from an Android phone. The PN532 is driven by a Adafruit Feather 32u4 single-board 
microcontroller. The Feather was selected because of it uses 3V logic, matching the PN532's 3V logic. The Arduino would 
have required a logic level shifter to shift its 5V logic down to 3V. The wiring of the NFC module hardware is shown in 
Figure~\ref{fig:nfc-module}.

The PN532 shield is run in Serial Peripheral Interface (SPI) mode by wiring SEL0 "off" and SEL1 "on". This interface 
uses the SCLK, MOSI, MISO, SSEL pins on the PN532 shield

\begin{figure}
    \includegraphics[width=\textwidth]{Diagrams/nfc_module}
    \caption{NFC Module Wiring Diagram}
    \label{fig:nfc-module}
\end{figure}
...
packets, starting with the packet flag, followed by the opcode. Unlike REQUEST packets, bytes 3 and 4 of an ACK packet 
denote the block number that is being acknowledged by the Pi. This ensures that the Pi correctly buffers the data in 
sequence. Figure~\ref{fig:ack-packet} demonstrates the format of ACK packets.
...
% n.b. get references after dinner

COBS eliminates occurences of the packet flag in the data by transforming them into pointers. Appended to the start of 
the data is a byte, whose value denotes the index of the next occurence of the packet flag, relative to that 
byte. If the next instance is not the terminal byte, then it will be transformed into a relative index of the next 
occurence of the packet flag. This process of replacing the packet flag with pointers is repeated until the terminal 
byte. Finally, the data is terminated with the packet flag. Table~\ref{tab:cobs-encoding} shows a few sample inputs 
drawn from actual data used to test the SBACS hardware, encoded using COBS.

\begin{table}[!h]
    \begin{tabular}{|c|c|c|} \hline
        Input Type & In Bytes & Out Bytes \\ \hline
        Module ID ""nfc"" & 6E 66 63 00 & 04 6E 66 63 00 7E \\ \hline
        PIN data & 30 34 28 31 & 04 30 34 28 31 7E \\ \hline
...
This algorithm was chosen for its consistent overhead. COBS inserts one additional byte for every 254 bytes in a data 
block, requiring the data to be divided into 254-byte sections if it is larger than that. Therefore, to reduce the
number of computations required to chunk the data for COBS and then for DATA packets, the DATA packet buffer size was 
chosen as 255 bytes to match COBS. This way, data that has been chunked for processing in COBS can be immediately 
inserted into a packet without further transformations.

% Remaining stuff after dinner: SETUP and ERROR packets
...
% Todo: run tests more than once and report min/max/avg?
...
In particular, the largest limitation was that there was no way to debug an Authentication Module by monitoring the 
data it was sending over USB, while also running the lock hardware's core process, as only one program at a time could 
access the authentication module's device file at a time. If the Arduino IDE Serial Monitor was running, the SBACS 
process could not. Since the Arduino IDE did not provide any other debugging mechanisms besides the Serial Monitor, 
...
%TODO: Summary of problem, our solution, and our accomplishments again (shorter than intro)

%TODO: Suggestions to extend the project, what else could be done? (new authentication modules)

%TODO: Alternative solutions found later that may have been better? (continous integration, delivery?)

The SBACS provides a fairly simple method of digitising any physical lock. The digital locks are secure, resuable, and
...
authentication, which would encourage developing a module that performs facial, fignerprint, or other biometric
