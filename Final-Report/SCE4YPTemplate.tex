...
A single-board computer, or SOC, incorporate all of the elements necessary for a functional 
...
circuit board. The main distinction between SOCs and SOMs is the same as that between computers and microcontrollers. A 
computer is a general-purpose computing device, and has more memory, a stronger CPU, and other improved hardware 
features to support the more intense computing needs of a user. A microcontroller is designed specifically for one 
purpose, such as powering an alarm clock. As such, it does not require a significant amount of memory, processing 
...
hardware abstraction of the hardware interface, and one class to handle server communication. Figure NEXT is a block 
diagram of the architecture of the lock software.
...
With an Adapter pattern in place to handle the creation of various types of authentication module, there also needed to 
...
% TODO: JavaScript with JQuery was used to perform client side processing (references - JS and JQuery)
%	JQuery specifically was chosen because of its ease of DOM manipulation and HTTP requests
...
\subsection{Lock Control Circuit} \label{section:lock-control-circuit-implementation}
...
\subsection{Learning to Use Amazon Web Services}
...
\subsection{Issues with String Encodings}
...
\subsection{Improving Response Time}
...
\subsection{Issues with Serial Communcation}
