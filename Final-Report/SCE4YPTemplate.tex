...
box, and receive their package. These interations are summarised in the use case diagram in Figure~\ref
...
a use case diagram outlining the interations to manage a long term storage facility with SBACS.
...
period. All of the attemped authentications could also be logged and made available to certain users, creating a
...
of installing and manageming the information technology parts of the system to other companies that might want to
...
Recall from section , the NFC communication method that we decided on used host-based card emulation. Android provides
...
% TODO: Reference: https://www.w3schools.com/html/html_intro.asp will span first few sentences
...
the content of a web page. Since this language is the standard, it was a natural choice for our
...
% TODO: Need reference for varying support of JS, then for JQuery functioning across browsers
% Reference for browser support: http://jquery.com/browser-support/
% JQuery makes things easier reference??: http://jquery.com/
...
are supported by different web browsers. There is also the added benefit of having functions that
...
standard JQuery functions. The support across many browsers and the added functionality make this a
...
% TODO: Explain what a single page application is (need references)
%	Then why we used it (educational experience, any benefits?)
% SPA Reference: https://msdn.microsoft.com/en-us/magazine/dn463786.aspx (maybe quote first sentence of it)

The web interface is designed to be a Single Page Application (SPA). A Single Page Application is
a web application that runs on a single web page, having no HTML page reloads. This means
...
% Salting Reference: pick between: https://crackstation.net/hashing-security.htm#salt and https://learncryptography.com/hash-functions/password-salting
...
% HTTP vs HTTPS Reference: https://blog.mozilla.org/security/2017/01/20/communicating-the-dangers-of-non-secure-http/
% TODO: Find reference about HTTP itself for first sentence (had trouble finding a good one so far)

The Hypertext Transfer Protocol (HTTP) is the standard protocol used to send requests on the internet.
...
and encryption between the client and server for every HTTPS request that is made. This ensures that
...
% Patterns to discuss include: Front Controller, MVC, Facade, 
% Architecture patterns: Client-Server, Basic Layer
...
methods, anonymous classes were used in all cases. Further, we decided that handling the parsing shoudl be done as
...
\subsection{Issues with Serial Communcation} \label{issues-with-serial-communcation}
