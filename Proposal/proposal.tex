...
\title{Sensor Based Access Control System}
\author{
	Craig Shorrocks \\
	100887781
	\and
...
This proposal outlines a Sensor Based Access Control System~(SBACS) that will expand upon such a concept to further tie
 security and identity to the electronics we use most: our phones. Using technologies like near-field communication~(NFC)
sensors and quick response~(QR) codes, the identity associated with a phone can be used as identification for anything.
This represents a huge advantage with respect to convenience, and it would even further lower the number of possible
failure points in security.
...
% Not sure how but I think we should remove "nonliteral quotes" here (and everywhere)
...
can only be reliably eavesdropped from a distance of approximately 10 meters or less if the interaction is between two active
devices, dropping to 1 metre if the interaction is a passive communication. This, along with some other facts, makes a
man-in-the-middle attack nearly impossible to accomplish in a real-world scenario~\autocite{NFCSECURITY}. These attacks
may be protected against by establishing a secure channel, by using symmetric-key encryption or other secret-sharing
method. For these reasons, NFC is a reliable method to pair a smart device with an electronic lock system.
...
% Possible ramifications for this system - might be of interest for the final report, not sure about the proposal: 
% - Parking lots will be less congested as people spend less time in stores
% - System will make it easier for customers with mobility impairments to obtain everything they need
%	- The system could also help with vision impaired customers
...
\begin{ganttchart}[
	...
	\ganttbar{Hardware Prototype}{5}{9} \\
	\ganttbar{Software Prototype}{5}{9} \\
	\ganttbar{Preliminary testing}{8}{11} \\
...
\begin{ganttchart}[
	...
	\ganttbar{Draft final report}{5}{12} \\
